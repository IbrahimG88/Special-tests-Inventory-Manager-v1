export const getConsumptionData = async (previousDate, nowDate) => {
  ...
  const res = await fetch(
    `${process.env.LIS_LAB_URL}/api3/integration/worklist/${previousDate.year}-${previousDate.month}-${previousDate.day}%20${previousDate.hours}:${previousDate.minute}:00:00/${nowDate.year}-${nowDate.month}-${nowDate.day}%20${nowDate.hours}:${nowDate.minute}:00:00`,
    { mode: "cors" }
  ).then((response) => {
  ...
  finalArray.push({
    name: "numberOfRegisteredPatients",
